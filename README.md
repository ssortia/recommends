...
- **RSS-источники** — добавление RSS/Atom-ленты по URL (`RssGate` над `rss-parser`), глобальный справочник источников с подписками пользователей (`Source`/`UserSource`), первичный парсинг и сохранение статей в единой транзакции; веб-страница `/sources`
