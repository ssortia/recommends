...
- **RSS- и Telegram-источники** — добавление RSS/Atom-ленты по URL (`RssGate` над `rss-parser`) или публичного Telegram-канала по `@username`/ссылке (`TelegramGate` через HTTP-scraping `t.me/s/`); тип определяется автоматически (`SourceInputParser`), общий справочник источников с подписками пользователей (`Source`/`UserSource`), первичный парсинг и сохранение статей в единой транзакции; веб-страница `/sources`
