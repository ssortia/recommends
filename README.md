...
- **RSS- и Telegram-источники** — добавление RSS/Atom-ленты по URL (`RssGate` над `rss-parser`) или публичного Telegram-канала по `@username`/ссылке (`TelegramGate` через HTTP-scraping `t.me/s/`); тип определяется автоматически (`SourceInputParser`), общий справочник источников с подписками пользователей (`Source`/`UserSource`), первичный парсинг и сохранение статей в единой транзакции; favicon источника подтягивается автоматически для RSS (`FaviconGate`) с фолбэком на иконку по типу; отписка от источника с подтверждением (`AlertDialog`), общие `Source`/`Article` сохраняются для других подписчиков; веб-страница `/sources`
- **Описание интересов** — свободный текст (до 1000 символов) в отдельной таблице `UserPreferences` (1:1 с `User`, ADR-014), задел под будущую фильтрацию по категориям и AI-оценку релевантности; редактируется на веб-странице `/sources`. Дополнительно — индивидуальное описание интересов для конкретного источника (таблица `SourcePreference`, составной ключ `userId`+`sourceId`, ADR-015): при наличии имеет приоритет над общим описанием для будущей AI-оценки; редактируется инлайн в карточке источника на `/sources`
