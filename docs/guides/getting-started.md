...
| `docker compose up -d db` падает с `port is already allocated` | Порт 5444 занят другим контейнером Postgres              | Использовать уже запущенный контейнер либо задать `DB_HOST_PORT` и выполнить `pnpm setup:worktree --force` — порт попадёт и в `DATABASE_URL` |
