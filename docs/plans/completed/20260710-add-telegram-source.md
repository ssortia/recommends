...
- [x] локальный HTTP-сервер в `telegram-source.spec.ts`, слушающий **фиксированный** порт (например,
...
- [x] задокументировать в `.env.example` и/или README dev-инструкции по e2e: перед прогоном e2e API
...
- [x] тест: добавление `@username` → канал появляется в списке
- [x] тест: повторное добавление того же канала → сообщение о дубликате
- [x] тест: разный регистр (`@Username` при первом добавлении, `t.me/username` при повторном) →
      трактуется как один и тот же источник (проверка нормализации регистра из Task 3)
- [x] запустить: `pnpm --filter @repo/web test:e2e` (или актуальная команда e2e из `package.json`),
      предварительно перезапустив API с `TELEGRAM_PREVIEW_BASE_URL` из шага выше — фактически
      выполнено: в песочнице уже был поднят docker-compose стенд (`recommends-api-1` с
      `TELEGRAM_PREVIEW_BASE_URL=http://172.22.0.1:4310`, `NODE_ENV=test`), mock-сервер слушал на
      всех интерфейсах (`0.0.0.0:4310`), чтобы быть доступным из контейнера API через docker-мост;
      все 3 новых теста прошли (`3 passed`). ⚠️ Обнаружена (не в рамках Task 9) гонка гидратации
      next-auth сессии в `AddSourceForm`: мгновенный submit сразу после перехода на `/sources` может
      уйти без `accessToken` — воспроизводится и в неизменённом `e2e/sources.spec.ts` (RSS) на этом
      же стенде, значит это существующая проблема, не регрессия от Task 9. В
      `telegram-source.spec.ts` добавлена `await page.waitForLoadState('networkidle')` перед
      заполнением формы как обходной путь; при желании стоит завести отдельную задачу на
      устранение гонки в самой форме.
...
- [x] обновить `docs/PRODUCT.md`, если нужно (пункт про Telegram уже есть, свериться с фактом) —
      сверено: раздел «Источники» и «Технические ограничения и допущения» уже точно описывают
      реализацию (публичные каналы — HTTP-scraping, приватные — MTProto/gramjs в беклоге),
      изменений не потребовалось
- [x] проверить, нужен ли новый ADR (архитектурное решение уже покрыто ADR-009 — новый не нужен) —
      подтверждено: `TelegramGate` реализован строго по паттерну Gate из ADR-009 (сырой результат
      или `null`, без доменных исключений), расхождений нет
- [x] обновить README.md, если список фич не отражает добавление Telegram-источников — обновлена
      строка «RSS-источники» → «RSS- и Telegram-источники» с упоминанием `TelegramGate` и
      `SourceInputParser`
