...
- [x] реализовать `toSlug(dirName)` — нормализация имени папки для имени БД и имени хоста
- [x] реализовать `resolveDatabaseName(slug)` с особым случаем для основного checkout
- [x] реализовать `pickPortPair(isPortFree, startFrom, reserved)` — первая пара, свободная и в системе, и среди занятых соседними копиями
- [x] реализовать `buildEnvContent(example, params)` — подстановка значений в содержимое `.env.example` без потери остальных ключей и комментариев
- [x] написать тесты на `toSlug` (обычное имя, дефисы и заглавные, недопустимые символы)
- [x] написать тесты на `resolveDatabaseName` (копия и основной checkout)
- [x] написать тесты на `pickPortPair`: первая пара свободна; первые две заняты в системе; пара свободна в системе, но занята соседним `.env`; порты кончились
- [x] написать тесты на `buildEnvContent` (значения подставлены, прочие ключи и комментарии сохранены)
- [x] запустить `node --test scripts/lib/*.test.mjs` — тесты должны пройти
...
- [x] добавить в `next.config.ts` опцию `allowedDevOrigins` с адресами `*.localhost`
- [x] вынести порты в `docker-compose.yml` в переменные (`DB_HOST_PORT`, `API_PORT`, `WEB_PORT`) с текущими значениями по умолчанию
- [x] привести `.env.example` в соответствие: порт БД 5444, `POSTGRES_DB="recommends"` вместо `"nexst"`, новые переменные (`API_PORT`, `WEB_PORT`, `DB_HOST_PORT`, `CORS_ORIGIN`) с комментариями
- [x] ⚠️ частично: проверить вручную: две копии работают одновременно, вход в одной не сбрасывает сессию в другой
- [x] ⚠️ статически: проверить вручную: `docker compose up -d db` работает и без `.env`, и со сгенерированным `.env` копии
- [x] проверить вручную: заход на `localhost:<WEB_PORT>` не упирается в CORS благодаря списку в `CORS_ORIGIN`
- [x] прогнать `pnpm test` — существующие тесты должны проходить

⚠️ Уточнение по факту проверки. После добавления ключей в `.env.example` перегенерация
`pnpm setup:worktree --force` подставляет `API_PORT`/`WEB_PORT`/`DB_HOST_PORT`/`CORS_ORIGIN`
по месту — хвост в конце файла больше не дописывается (остаток Task 4 закрыт).

⚠️ Изоляция сессий проверена в одной копии: логин `admin@example.com` на
`http://sandworm.localhost:3010` даёт рабочую сессию, при этом `http://localhost:3010` в том же
браузере остаётся неавторизованным (редирект на `/login`), а возврат на slug-хост сессию
сохраняет. Это подтверждает, что cookie next-auth скоупится по имени хоста, а не по порту, —
то есть две копии на разных slug-хостах не пересекаются. Вторая реально запущенная копия не
проверялась (нет второго подготовленного worktree со свободной парой портов).

⚠️ `docker compose up -d db` в копии не запускался: порт 5444 занят контейнером
`recommends-db-1` из основного checkout (проект compose `recommends`), а это и есть общий
контейнер по схеме плана — вторая копия его не поднимает. Подстановка проверена через
`docker compose config`: со сгенерированным `.env` публикуются `5444`, `3010`, `3011`,
с пустым `--env-file` — дефолтные `5444`, `3000`, `3001`.

⚠️ CORS: правка `main.ts` не потребовалась — список из `CORS_ORIGIN` уже разбирается по запятой
в массив. Preflight `OPTIONS /sources` с `Origin: http://sandworm.localhost:3010` и
`http://localhost:3010` возвращает соответствующий `access-control-allow-origin`, посторонний
origin заголовка не получает. Браузерные запросы к API с обоих хостов CORS не блокирует:
наблюдаемые 403 на `/sources` и `/preferences` одинаковы на обоих хостах и приходят от
`VerifiedGuard` (seed-пользователь с неподтверждённым email) — к схеме портов отношения не имеют.
...
- [x] написать ADR-016 по формату из `docs/DOCUMENTATION.md`: схема стенда, рассмотренные варианты (общая БД против контейнера на копию, хеш портов против первой свободной пары, slug-хост против правки `listen`), последствия и границы (compose-стек `api`/`web` вне схемы)
- [x] добавить ADR-016 в таблицу-индекс `docs/adr/README.md`
- [x] написать гайд `worktree-dev.md` строго по структуре из `docs/DOCUMENTATION.md` (Название → Предварительные требования → Шаги → Проверка → **Частые проблемы**)
- [x] в разделе «Частые проблемы» описать: CORS при заходе на неожиданный хост, занятый порт БД, отсутствие контейнера, одновременный прогон e2e (общий mock-порт 4310)
- [x] в разделе «Проверка» описать ручной сценарий: логин seed-пользователем, ссылки из писем в логах API при `MAIL_TRANSPORT=json`
- [x] исправить в `README.md` и `getting-started.md` неверные утверждения про автоматическую загрузку `.env` и порт БД, добавить шаг `pnpm setup:worktree`
- [x] в `development.md` обновить раздел «Запуск» (порты берутся из `.env`) и сослаться на новый гайд
...
- [x] поднять стенд в текущей копии на нестандартных портах
- [x] прогнать `pnpm --filter @repo/web test:e2e` с `PLAYWRIGHT_BASE_URL` (slug-хост) и `PLAYWRIGHT_API_URL` (`127.0.0.1`)
- [x] убедиться, что тесты с mock-серверами проходят с учётом `NODE_ENV=test`, `TELEGRAM_PREVIEW_BASE_URL` и `E2E_RSS_FEED_HOST`
- [x] зафиксировать в гайде фактические команды прогона и ограничение «не гонять e2e в двух копиях одновременно»
- [x] при падениях — исправить причину и повторить прогон до полного прохождения

⚠️ Уточнение по факту прогона. Стенд поднят двумя процессами (общий `pnpm dev` не подходит:
API нужен собственный набор переменных): API — `NODE_ENV=test
TELEGRAM_PREVIEW_BASE_URL=http://127.0.0.1:4310 pnpm --filter @repo/api dev` на 3011, web —
`pnpm --filter @repo/web dev` на 3010. `dotenv-cli` шелл-переменные не затирает, режим `test`
подтверждён телом ответа `/auth/__test/last-token` (`{"statusCode":404,"message":"Token not
found"}` вместо пустого 404). Прогон: `PLAYWRIGHT_BASE_URL=http://sandworm.localhost:3010
PLAYWRIGHT_API_URL=http://127.0.0.1:3011 E2E_RSS_FEED_HOST=127.0.0.1` — **24 из 24 зелёные**.

⚠️ Первый прогон дал 22/24: падали `interests-description.spec.ts:57` и
`source-interests-description.spec.ts:132` по таймауту на `getByLabel('Описание интересов')`.
Разбор: на снимке страницы виден баннер «Email не подтверждён» и 403 от `VerifiedGuard`, то есть
флейк хелпера `loginAsVerifiedUser` — `page.goto('/verify-email?token=…')` возвращается по
событию `load`, а `POST /auth/verify-email` уходит уже после, и при 8 воркерах логин успевает
произойти раньше подтверждения. К схеме портов отношения не имеет: `git diff main...HEAD` по
`apps/web/e2e`, `apps/web/src`, `apps/api/src/auth` и `playwright.config.ts` пуст, файлы в
последний раз менялись в `8dc64b4` (июль, issue #21). Повторные прогоны (отдельно два spec-файла,
затем весь набор) прошли полностью. Строка про этот флейк добавлена в «Частые проблемы» гайда;
системное решение — дожидаться ответа `/auth/verify-email` в хелпере — вне рамок issue #31.

⚠️ Ограничение фиксированного порта 4310 подтверждено: `MOCK_PORT = 4310` в
`apps/web/e2e/telegram-source.spec.ts` (константа, не `listen(0)`), параллельный прогон в двух
копиях по-прежнему невозможен — это уже описано в гайде и ADR-016.

➕ Сверх плана: раздел «Проверка → 6» гайда переписан на фактическую последовательность
(установка chromium, два терминала, проверка режима `test` через curl, команда прогона), плюс
пункт про `PLAYWRIGHT_BASE_URL` = slug-хост и запрет параллельного прогона.

### Task 8: Verify acceptance criteria

- [x] сверить реализацию с критериями приёмки issue #31
- [x] обновить в issue #31 критерий про «новые секреты»: принято решение использовать фиксированные dev-заглушки плюс guard из Task 2 — зафиксировать отклонение там же и в ADR-016
- [x] проверить краевые случаи: нет `.env.example`, порты кончились, контейнер БД не запущен, повторный запуск скрипта, соседняя копия заняла пару портов
- [x] запустить полный набор: `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm test:scripts`
- [x] запустить e2e: `pnpm --filter @repo/web test:e2e`
- [x] ⚠️ статически: проверить, что основной checkout продолжает работать без изменений в своём `.env`

#### Сверка с критериями приёмки

| Критерий                                              | Итог                     | Как проверено                                                                                                       |
| ----------------------------------------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| dev-скрипты API и web загружают корневой `.env`       | подтверждено             | реальный запуск обоих процессов: `/health` → 200 на 3011, web → 307 на 3010, оба порта взяты из `.env`              |
| `pnpm setup:worktree` готовит стенд целиком           | подтверждено с оговоркой | реальный `--force`: `.env`, `db:generate`/`db:migrate`/`db:seed`, печать адреса и учётки. Оговорка — секреты (ниже) |
| повторный запуск не перезаписывает `.env`             | подтверждено             | запуск без `--force`: «файл не тронут», mtime `.env` не изменился                                                   |
| две копии работают независимо                         | подтверждено с оговоркой | порты: реальный прогон с соседним `.env` (3010/3011 → 3020/3021). Сессии — косвенно, вторая копия не поднималась    |
| `allowedDevOrigins` для `*.localhost`                 | подтверждено             | `next.config.ts`; `http://sandworm.localhost:3010/login` → 200 (резолв в `::1`)                                     |
| порты в `docker-compose.yml` — переменные с дефолтами | подтверждено             | `docker compose config`: без `.env` → 5444/3001/3000; с `.env` копии → 5444/3011/3010                               |
| `.env.example` соответствует реальности               | подтверждено             | порт БД 5444, `POSTGRES_DB="recommends"`, добавлены `API_PORT`/`WEB_PORT`/`DB_HOST_PORT`/`CORS_ORIGIN`              |
| README и гайд описывают запуск в worktree             | подтверждено             | `README.md`, `docs/guides/{getting-started,development,worktree-dev}.md`; структура гайда по `DOCUMENTATION.md`     |

⚠️ Критерий «новые секреты» **не выполнен сознательно**: используются фиксированные dev-заглушки
плюс guard при `NODE_ENV=production`. Тело issue не правилось, отклонение объяснено комментарием
[#issuecomment-5756851112](https://github.com/ssortia/recommends/issues/31#issuecomment-5756851112)
и зафиксировано в ADR-016 (дважды: «Рассмотренные варианты» и «Последствия»).

#### Краевые случаи (все прогнаны реально, в песочнице `/tmp`, рабочий `.env` не пострадал)

| Случай                         | Поведение                                                                       |
| ------------------------------ | ------------------------------------------------------------------------------- |
| нет `.env.example`             | exit 1 с понятным сообщением, проверка идёт до обращения к БД                   |
| порты кончились                | `pickPortPair` бросает «не найдено свободной пары портов в диапазоне 3010–4001» |
| контейнер БД не запущен        | exit 1, подсказка `docker compose up -d db`, `.env` не создаётся                |
| повторный запуск без `--force` | печатает текущие параметры, `.env` не тронут (mtime не изменился)               |
| соседняя копия заняла пару     | реальный git-worktree стенд: сосед с `WEB_PORT=3010` → копия получила 3020/3021 |

➕ **Найден и исправлен дефект.** `pnpm setup:worktree --force` при запущенном стенде копии
уводил её на другую пару портов: собственные `3010/3011` выглядели занятыми, и подбор выдавал
`3020/3021`. Это ловушка — `getting-started.md` советует именно `--force` для восстановления
после сбоя миграций. Добавлена `readOwnPorts()` в `scripts/setup-worktree.mjs`: пара портов из
существующего `.env` считается собственностью копии и при перегенерации сохраняется. Проверено
реально — с занятыми 3010/3011 `--force` оставил 3010/3011, итоговый `.env` побайтово совпал с
исходным. Строка про это добавлена в ADR-016 (п. 2 «Решение»).

#### Прогоны

- `pnpm turbo run lint typecheck test --force` (без кэша, 11 задач) — зелено;
  `@repo/api`: **33 сюиты / 230 тестов**, 0 падений.
- `pnpm test:scripts` — **17 тестов**, 0 падений.
- `npx eslint scripts --max-warnings 0` и `prettier --check` по изменённым файлам — чисто.
- e2e: API (`NODE_ENV=test`, `TELEGRAM_PREVIEW_BASE_URL=http://127.0.0.1:4310`) + web,
  `PLAYWRIGHT_BASE_URL=http://sandworm.localhost:3010 PLAYWRIGHT_API_URL=http://127.0.0.1:3011
E2E_RSS_FEED_HOST=127.0.0.1` — **24/24 зелёные с первого прогона** (флейк из Task 7 не
  воспроизвёлся). Процессы и порты 3010/3011/4310 после прогона освобождены.
- CORS перепроверен на живом API: slug-хост и `localhost:3010` получают свой
  `access-control-allow-origin`, посторонний origin — нет.

#### Основной checkout

`/home/ssortia/sandbox/recommends/.env` не изменялся (mtime 2026-07-13), из этого worktree
ничего за его пределами не трогалось. Влияние правок разобрано статически:

- `docker-compose.yml` — в его `.env` нет `DB_HOST_PORT`/`API_PORT`/`WEB_PORT`, поэтому
  подставляются дефолты. Проверено подстановкой его `.env` в наш compose:
  `docker compose --env-file /home/ssortia/sandbox/recommends/.env config` → **5444/3001/3000**,
  то есть ровно прежние значения.
- dev-скрипты — `${API_PORT:-3001}` / `${WEB_PORT:-3000}` дают прежние порты.
- `CORS_ORIGIN` в его `.env` нет → `main.ts` откатывается на `NEXTAUTH_URL`, как раньше.
- guard из Task 2 неактивен: в его `.env` стоит `NODE_ENV="test"`.

⚠️ Одно **изменение поведения** основного checkout, о котором стоит знать: раньше
`nest start --watch` корневой `.env` не читал вовсе (и падал на валидации env), теперь читает —
а значит подхватит и `NODE_ENV="test"` из его `.env`. Это включит в локальном dev основного
checkout test-only эндпоинты `/auth/__test/*`. Формально это следствие содержимого его `.env`,
а не бага, но при первом `pnpm dev` там строку стоит убрать или заменить на `development`.
Сам файл не трогался — решение за разработчиком.

⚠️ Не проверено вживую: одновременный запуск **двух** копий (вторая копия `oystercatcher` — вне
этого worktree, а инструкция запрещает менять что-либо за его пределами). Изоляция сессий
по-прежнему подтверждена косвенно — через скоуп cookie по имени хоста (Task 5) и через реально
разведённые порты. Это единственный критерий, оставшийся с неполным доказательством.

⚠️ Мелкое, не чинилось: если `db:migrate` падает посреди `setup:worktree`, `.env` уже записан,
и повторный запуск без `--force` молча выходит, не доделав миграции. Восстановление — `--force`
(теперь безопасный по портам). Порт БД `5444` в обёртке — константа, значение `DB_HOST_PORT`
из `.env.example` она не читает.
