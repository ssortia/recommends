...
- [ ] сверить с критериями приёмки issue #7: валидация URL, проверка доступности/валидности фида,
      появление в списке, сообщение о дубликате, автозапуск первичного парсинга
- [ ] проверить edge cases: URL без протокола, редирект 301, фид с пустым `<channel>`, повторное
      добавление тем же и другим пользователем
- [ ] run full test suite: `pnpm test`
- [ ] run `pnpm typecheck` и `pnpm lint`
- [ ] проверить, что миграция и seed воспроизводимы на чистой БД

### Task 10: [Final] Обновить документацию

- [ ] обновить `README.md` (структура проекта — добавить модуль `sources` в список `apps/api/src`)
- [ ] проверить/обновить `CLAUDE.md`, если обнаружен новый паттерн, требующий фиксации
