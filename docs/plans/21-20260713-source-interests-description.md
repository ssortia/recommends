...
- [ ] тест: добавить источник, открыть «Интересы» в карточке, сохранить описание, проверить что значение сохраняется после перезагрузки страницы
- [ ] тест: очистка описания (пустое значение) сохраняется и не ломает общее описание интересов (#12)
- [ ] тест: описание не пересекается между двумя разными источниками одного пользователя
- [ ] run e2e: `pnpm --filter @repo/web test:e2e` (или актуальная команда проекта)
- [ ] run full unit test suite: `pnpm test` — все тесты проходят
