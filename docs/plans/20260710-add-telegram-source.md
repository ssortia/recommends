...
- [ ] обновить `label`/`placeholder` поля (например, «Источник» /
      «https://example.com/feed.xml или @channel»)
- [ ] обновить текст ошибки 400 на общий («Не удалось добавить источник: проверьте формат ссылки
      или имени канала»)
- [ ] вручную проверить в браузере (`pnpm dev`), что форма отправляет и `@username`, и RSS URL
